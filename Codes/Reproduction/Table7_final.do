...
/*Sets base directory*/
/*********************/
global project "/Users/abdoulayecisse/Desktop/Revised-reproduction-package-for-Acemoglu-et-al.-2019/"  /* Set base directory                         */
...
#delimit ;
estout eOLS1 eOLS2 eOLS3 eOLS4 eOLS5 eOLS6 eOLS7 eOLS8 eOLS9   using "$project/Results/Reproduction/TableChannelsTop.tex", style(tex) 
varlabels(dem "Democracy" ) 
cells(b(star fmt(%9.3f)) se(par)) 
stats( N N_g, fmt(%7.0f %7.0f) labels( "\input{Results/Reproduction/TableChannelsTop_Add} Observations " "Countries in sample"))  
keep(dem ) order(dem )  stardrop(dem)
collabels(none)   nolabel replace mlabels(none);
#delimit cr

#delimit ;
estout eOLS1_add eOLS2_add eOLS3_add eOLS4_add eOLS5_add eOLS6_add eOLS7_add eOLS8_add eOLS9_add  using "$project/Results/Reproduction/TableChannelsTop_Add.tex", style(tex) 
...
#delimit ;
estout eIV1 eIV2 eIV3 eIV4 eIV5 eIV6 eIV7 eIV8 eIV9   using "$project/Results/Reproduction/TableChannelsMid.tex", style(tex) 
varlabels(dem "Democracy" ) 
cells(b(star fmt(%9.3f)) se(par)) 
stats( widstat jp  N N_g  , fmt(%7.1f %7.2f %7.0f %7.0f ) labels( "\input{Results/Reproduction/TableChannelsMid_Add} Exc. instruments F-stat." "Hansen p-value"  Observations "Countries in sample" ))    
keep(dem ) order(dem )  stardrop(dem) 
nolabel replace mlabels(none) collabels(none) ;
#delimit cr

#delimit ;
estout eIV1_add eIV2_add eIV3_add eIV4_add eIV5_add eIV6_add eIV7_add eIV8_add eIV9_add  using "$project/Results/Reproduction/TableChannelsMid_Add.tex", style(tex) 
varlabels(longrun "Long-run effect of democracy"  effect$limit "Effect of democracy after $limit years"  persistence "Persistence of outcome process") 
cells(b(star fmt(%9.3f)) se(par)) 
keep(longrun effect$limit persistence) 
order(longrun effect$limit persistence)
...
#delimit ;
estout eIVb1 eIVb2 eIVb3 eIVb4 eIVb5 eIVb6 eIVb7 eIVb8 eIVb9   using "$project/Results/Reproduction/TableChannelsBot.tex", style(tex) 
varlabels(dem "Democracy" ) 
cells(b(star fmt(%9.3f)) se(par)) 
stats( N , fmt(%7.0f ) labels( "\input{Results/Reproduction/TableChannelsBot_Add} Observations" ))    
keep(dem ) order(dem )  stardrop(dem) 
nolabel replace mlabels(none) collabels(none) ;
#delimit cr

#delimit ;
estout eIVb1_add eIVb2_add eIVb3_add eIVb4_add eIVb5_add eIVb6_add eIVb7_add eIVb8_add eIVb9_add  using "$project/Results/Reproduction/TableChannelsBot_Add.tex", style(tex) 
varlabels(longrun "Long-run effect of democracy"  effect$limit "Effect of democracy after $limit years"  persistence "Persistence of outcome process") 
cells(b(star fmt(%9.3f)) se(par)) 
keep(longrun effect$limit persistence) 
order(longrun effect$limit persistence)
