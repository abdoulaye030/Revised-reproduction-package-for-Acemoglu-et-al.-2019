...
#delimit ;
estout e1 e2 e3 e4 e5 e6 e7 e8 e9  using "$project/Results/Reproduction/Table2SLSTop.tex", style(tex) 
varlabels(dem "Democracy" ) 
cells(b(star fmt(%9.3f)) se(par)) 
stats( jp N N_g widstat , fmt(%7.2f %7.0f %7.0f %7.1f) labels( "\input{Results/Reproduction/Table2SLSTop_Add} Hansen p-value" "Observations" "Countries in sample"  "Exc. Instruments F-stat." ))   
keep(dem) order(dem) stardrop(dem) 
nolabel replace mlabels(none) collabels(none) ; 
#delimit cr

#delimit ;
estout e1_add e2_add e3_add e4_add e5_add e6_add e7_add e8_add e9_add using "$project/Results/Reproduction/Table2SLSTop_Add.tex", style(tex) 
...
#delimit ;
estout e1fs e2fs e3fs e4fs e5fs e6fs e7fs e8fs e9fs using "$project/Results/Reproduction/TableFS.tex", style(tex) 
...
#delimit ;
estout e1 e2 e3 e4 e5 e6 e7 e8 e9 using "$project/Results/Reproduction/Table2SLSbot.tex", style(tex) 
varlabels(dem " Democracy" ) 
cells(b(star fmt(%9.3f)) se(par)) stats( N , fmt(%7.0f) labels( "\input{Results/Reproduction/Table2SLSBot_Add}  Observations"))   
keep(dem) order(dem) stardrop(dem) 
nolabel replace mlabels(none)  collabels(none);  
#delimit  cr

#delimit ;
estout e1_add e2_add e3_add e4_add e5_add e6_add e7_add e8_add e9_add using "$project/Results/Reproduction/Table2SLSBot_Add.tex", style(tex) 
varlabels(longrun "Long-run effect of democracy"  effect$limit "Effect of democracy after $limit years"  persistence "Persistence of GDP process") 
cells(b(star fmt(%9.3f)) se(par)) 
keep(longrun effect$limit persistence) 
order(longrun effect$limit persistence)
